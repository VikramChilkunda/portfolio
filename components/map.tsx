...
export default function Map() {
    ...
    return (
        ...
        /> */}
            ...
            <Marker position={losAngelesLocation}>
                <Popup>
                    <MyMarker />
                    {/* <ProjectCard
                        title={"Vehicle Description Generator"}
                        number={1}
                        description={"Automating compelling descriptions of vehicles for car dealerships."}
                        tags={[".NET", "C#"]}
                        imageURL={"nowcom.jpg"}
                        link={""}
                        scaleOnHover={false}
...
export const MyMarker = () => {
    for(let loc of Object.keys(groupedByLocation)) {
        let current = groupedByLocation[loc]
        console.log("current: ", current)
        return (
            <div>
                {
                    <ScrollableContainer items={current} />
                }
            </div>
        )
    }
